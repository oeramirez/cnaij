...
public class CucumberTestSteps {
	...
	public void checkServiceRunning(String serviceName) {
		ResponseEntity<String> healthResponse =
				restTemplate.getForEntity("/actuator/health", String.class, new HashMap<>());
	...
	public void callService(String serviceName, int prodId) throws Throwable {
		productResponse = this.restTemplate
				.getForEntity("/" + serviceName + "/" + prodId, Product.class, new HashMap<>());
	}
	...
	public void callServiceWithNonExistingProductId(String serviceName, int prodId) {
		errResponse = this.restTemplate
				.getForEntity("/" + serviceName + "/" + prodId, String.class, new HashMap<>());
	}
	
	@Then("return a (\\d+) not found status")
	public void returnAStatus(int status) {
		Assert.assertEquals(status, errResponse.getStatusCodeValue());
	}
	
	@And("return error message \"(.*)\"")
	public void returnErrorMessageIndicatingProductNotFound(String errMsg) {
